      ...
      <div className="relative z-10">
        ...
        <div className="mx-auto max-w-4xl px-4 py-8">
          ...
          <div className={`mb-12 text-center transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
            ...
            <h1 className="mb-4 text-4xl font-bold text-brand-mint glow-text md:text-6xl">
              Dev Wrapped {currentYear}
            </h1>
            <p className="text-xl text-muted-foreground">
              You debugged. You listened. You thrived.
            </p>
          </div>
          ...
          {stats.episodesCompleted === 0 ? (
            ...
            <div className="space-y-8">
              {/* Top Stats Grid */}
              <div className={`grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4 transition-all duration-1000 delay-300 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                <Card className="border-brand-mint/20 bg-card/50 hover:bg-card/70 transition-colors">
                ...
                <Card className="border-brand-mint/20 bg-card/50 hover:bg-card/70 transition-colors">
                  ...
                  <CardContent>
                    <div className="text-lg font-bold text-brand-mint truncate">
                      {stats.mostListenedGuest || 'Various'}
                    </div>
                  </CardContent>
                </Card>
              ...
              {stats.favoriteEpisode && (
                <Card className={`border-brand-mint/20 bg-card/50 transition-all duration-1000 delay-500 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                  <CardHeader>
                  ...
                  <CardContent>
                    <div className="flex flex-col gap-4 md:flex-row md:items-center">
                      ...
                      <Button
                        onClick={() => navigate(`/episodes/${stats.favoriteEpisode.slug}`)}
                        className="bg-brand-mint text-black hover:bg-brand-mint-dark"
                      >
                        Replay Episode
                      </Button>
              ...
              {stats.topTags.length > 0 && (
                <Card className={`border-brand-mint/20 bg-card/50 transition-all duration-1000 delay-700 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                  <CardHeader>
                        ...
                        <Badge
                          ...
                          className={`text-sm ${
                            index === 0
                              ...
                              : index === 1
                              ? 'bg-brand-mint/70 text-black'
                              : 'bg-brand-mint/40 text-white'
                          }`}
                        >
                          #{index + 1} {tag}
                        </Badge>
              ...
              {/* Achievements */}
              <Card className={`border-brand-mint/20 bg-card/50 transition-all duration-1000 delay-900 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {stats.achievements.map((achievement, index) => (
                      <div
                        key={index}
                        className="flex items-center gap-3 rounded-lg border border-brand-mint/20 bg-brand-mint/5 p-3"
                      >
              ...
              {/* Next Year Goals */}
              <Card className={`border-brand-mint/20 bg-card/50 transition-all duration-1000 delay-1100 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                <CardHeader>
                  <CardTitle>Start Your {currentYear + 1} Journey</CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="mb-4 text-muted-foreground">
                    Ready to level up your dev game? Discover more episodes and continue your learning journey.
                  </p>
