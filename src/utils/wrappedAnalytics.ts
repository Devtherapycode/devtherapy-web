...
export const getWrappedData = (): ListeningStats => {
  ...
  completedEpisodes.forEach((episodeId: string) => {
    const episode = allEpisodes.find(ep => ep.id === episodeId);
    if (episode?.guest) {
      const guestName = typeof episode.guest === 'string' ? episode.guest : episode.guest.name;
      guestCounts[guestName] = (guestCounts[guestName] || 0) + 1;
    }
  });

  const mostListenedGuest = Object.keys(guestCounts).reduce((a, b) => 
    guestCounts[a] > guestCounts[b] ? a : b, 
  ...
  const streakDays = lastListenDate && 
    new Date().getTime() - new Date(lastListenDate).getTime() < 7 * 24 * 60 * 60 * 1000 ? 7 : 0;
